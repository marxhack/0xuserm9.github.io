...
def write_post(post, template):
    values = {
        ...
        "END": (
            f"          <p>{esc(post['end'])}</p>"
            if post["end"] else "          <p>Thanks for reading.</p>"
        ),
    }
...
def main():
    ...
    stale = {p.name[:-5] for p in OUT.glob("*.html")}
    stale -= {p["slug"] for p in posts} | RESERVED_SLUGS
    if stale:
        print("\n  Orphaned HTML with no Markdown source (delete by hand if unwanted):")
